...
module.exports = {
  ...
  get: {
    ...
    info: function(req, res) {
      ...
      var data = {
        apiVersion: req.app.get('apiVersion'),
    ...
    kill: function(req, res) {
      ...
      if (!_.isNil(process.env.UNKILLABLE)) {
        debug('received kill request, but we are UNKILLABLE');
        return res.redirect('/');
      }
      ...
      if (killId == req.app.get('instanceIndex')) {
        ...
        debug('received kill request for this instance ' + req.app.get('instanceId') + ' (index: ' + killId + ')');
        ...
        debug('kill request for index ' + killId + ' received by instance ' + req.app.get('instanceId') + ' (index: ' + req.app.get('instanceIndex') + ')');
        if (killId) {
          debug('sending request with X-CF-APP-INSTANCE: ' + req.app.get("applicationId") + ":" + killId);
          ...
          request(options, function(err, response, body) {
            debug('sent X-CF-APP-INSTANCE request to index ' + killId);
            res.redirect('/');
          });
